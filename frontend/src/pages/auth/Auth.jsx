...
function getStrength(pw) {
  ...
  if (pw.length >= 10 && hasLetter && hasNumber && hasSpecial) {
    return { level: 4, key: "white-hot", label: "Strong" };
  }
  if (pw.length >= 8 && hasLetter && hasNumber) {
    return { level: 3, key: "hot", label: "Good" };
  }
  if (pw.length >= 6) {
    return { level: 2, key: "warm", label: "Okay" };
  }
  return { level: 1, key: "cold", label: "Weak" };
}
...
export default function Auth() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const valid = showLogin ? LogincircuitComplete : RegistercircuitComplete;
      ...
      {!showLogin ? (
              ...
              {form.password && (
                <div className="tf-gauge">
                  <div className="tf-gauge-track">
                    <div className={`tf-gauge-seg ${strength.level >= 1 ? strength.key : ""}`} />
                    <div className={`tf-gauge-seg ${strength.level >= 2 ? strength.key : ""}`} />
                    <div className={`tf-gauge-seg ${strength.level >= 3 ? strength.key : ""}`} />
                    <div className={`tf-gauge-seg ${strength.level >= 4 ? strength.key : ""}`} />
                  </div>
                  <div className="tf-gauge-label">
                    Strength: <span className="val">{strength.label}</span>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="tf-form-panel">
          <form className="tf-card" onSubmit={handleSubmit}>
            ...
            <h2 className="tf-card-heading">Sign in to your account</h2>
            <p className="tf-card-sub">
